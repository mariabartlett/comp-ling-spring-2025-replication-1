...
## What the data looks like at this point {.smaller}
...
- Had some issues (ultimately got to work) with `sophistication` package because outdated on GitHub

- One package they use is no longer maintained on CRAN

- We were able to replicate feature selection results, model results, and key plots

- Criticism: no training/test approach; compare to FRE score as a form of validation
...
- Clarity and additional data checks to walk through the process

- Modernized some of the approach to use more of the tidyverse

Substantive updates:

- Could run on a different poli sci text (e.g., campaign speeches)

- Could consider other machine learning approaches besides random forest (both for feature selection and main model)

- Ourselves: Including some measure in tone in covariates (are mor)

- More resources: Switch out outcome variable in BT model (instead of ease change to sentiment score; this would require new coders)
